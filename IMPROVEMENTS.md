...
- [x] Dependency-driven parallel execution: steps with `depends_on` run in waves via `asyncio.gather()`
...
### Fix 1: Async tools crash sync `stream()` with "does not support sync invocation"
- [x] Root cause: LangGraph's sync `stream()` path cannot invoke async tool coroutines directly
- [x] Fix: added `_sync_messages_stream()` bridge in `ResearchAgent` — runs `astream()` on a dedicated background thread via `asyncio.run_coroutine_threadsafe()`, then yields events synchronously to the caller
- [x] Streamlit and any other sync callers now get real streaming without blocking the event loop

### Fix 2: Plan-and-execute tools fail due to asyncio cross-loop errors
- [x] Root cause: tool coroutines scheduled on the main loop were being awaited from a different thread's loop, raising `RuntimeError: Task attached to a different loop`
- [x] Fix: plan-execute graph nodes use `asyncio.run_coroutine_threadsafe()` to submit tool coroutines to the agent's dedicated event loop, then block on the returned `Future` — keeping all async work on a single loop
