...
# default threadpool because the helpers are sync-only.
# ───────────────────────────────────────────────────────────────────
...
def list_all_sessions(agent: ResearchAgent = Depends(get_agent)) -> List[SessionSummary]:
...
def get_session(session_id: str, agent: ResearchAgent = Depends(get_agent)) -> SessionDetail:
...
def remove_session(session_id: str, agent: ResearchAgent = Depends(get_agent)) -> DeleteResult:
