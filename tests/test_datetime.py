...
class TestDatetimeCalculator:
    ...
    async def test_now(self):
        from src.tools.datetime_tool import datetime_calculator

        query = json.dumps({"operation": "now", "timezone": "UTC"})
        result = await datetime_calculator(query)
    ...
    async def test_simple_string_input(self):
        ...
        result = await datetime_calculator("what time is it")
    ...
    async def test_invalid_json(self):
        ...
        result = await datetime_calculator("{invalid json")
        assert "Error" in result
