...
class TestLengthConversions:
    ...
    async def test_inches_to_cm(self):
...
class TestWeightConversions:
    ...
    async def test_kg_to_pounds(self):
        result = convert("1 kg to pounds")
        assert "2.2" in result

    async def test_pounds_to_kg(self):
...
class TestTemperatureConversions:
    ...
    async def test_freezing_c_to_f(self):
        result = convert("0 celsius to fahrenheit")
        assert "32" in result

    async def test_boiling_c_to_f(self):
        result = convert("100 celsius to fahrenheit")
        assert "212" in result

    async def test_body_temp_f_to_c(self):
...
class TestEdgeCases:
    ...
    async def test_invalid_unit(self):
