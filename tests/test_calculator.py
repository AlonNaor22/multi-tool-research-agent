...
class TestMathFunctions:
    ...
    def test_sin_of_zero(self, calc):
        assert calc.calculate("sin(0)") == "0"

    def test_sin_of_pi_over_2(self, calc):
...
class TestSecurity:
    ...
    def test_blocks_import(self, calc):
        result = calc.calculate("__import__('os')")
        assert "Error" in result

    def test_blocks_dunder(self, calc):
        result = calc.calculate("__builtins__")
        assert "Error" in result

    def test_blocks_exec(self, calc):
        result = calc.calculate("exec('print(1)')")
        assert "Error" in result

    def test_blocks_eval(self, calc):
        result = calc.calculate("eval('1+1')")
        assert "Error" in result

    def test_blocks_open(self, calc):
...
class TestErrorHandling:
    ...
    def test_help_command(self, calc):
...
class TestModuleLevelFunction:
    ...
    def test_calculate_function_works(self):
